            ...
            {steps.map((step, index) => (
                <div key={index} className="group relative pt-8">
                    ...
                    <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-lg shadow-gray-200/50 hover:shadow-xl transition-all duration-300 h-full flex flex-col items-start relative overflow-hidden group-hover:-translate-y-2">
                        ...
                        <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${step.gradient} ${step.shadow} flex items-center justify-center mb-6 relative z-10 transform group-hover:rotate-6 transition-transform duration-500`}>
                            {step.icon}
                        </div>

                        <span className="text-6xl font-display font-black text-gray-100 absolute -right-2 -bottom-4 select-none group-hover:text-gray-200 transition-colors duration-300">
                            {step.id}
                        </span>

                        <h4 className="text-xl font-display font-black text-gray-900 mb-2 relative z-10">{step.title}</h4>
            ...
            <div className="bg-gray-900 rounded-3xl p-8 overflow-hidden relative shadow-2xl shadow-gray-900/30">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-12 border-t border-gray-800 pt-8">
                    {[
                        ...
                        { label: "Experiencia Usuario", val: "4.9/5", icon: <Heart size={16} className="text-rose-500 fill-rose-500" /> },
                        { label: "Atención Real", val: "24/7", icon: <Clock size={16} className="text-blue-400" /> }
                    ].map((metric, i) => (
                        <div key={i} className="text-center">
                             <div className="flex items-center justify-center gap-2 mb-2 text-white">
                                {metric.icon}
                                <span className="font-display font-black text-xl md:text-2xl tracking-tight whitespace-nowrap">{metric.val}</span>
                             </div>
